...
import { fetchGiftEntities, isPaidCard } from "./fetchGiftEntities";
import { validateEntities, validateSchedule } from "./validateEntities";
import { handlePaidGift } from "./handlePaidGift";
import { handleFreeGift } from "./handleFreeGift";

const TOLERANCE_MS = 60_000;

// const createSendGiftForUserIntoDB = async (
//     user: JwtPayload,
//     payload: ISendGift
// ) => {
...
const createSendGiftForUserIntoDB = async (
    ...
    const { card, sender, receiver } = await fetchGiftEntities(payload);

    // 2. Validate
    validateEntities(card, sender, receiver, payload);
    const scheduleDateTime = validateSchedule(
        payload.bookingDate,
        payload.bookingTime
    );

    // 3. Attach receiver ID
    if (receiver) payload.receiverId = receiver._id as Types.ObjectId;

    // 4. Handle paid or free
    if (isPaidCard(card)) {
        return handlePaidGift(payload, card, sender, receiver);
    }

    return handleFreeGift(payload, sender, receiver, scheduleDateTime);
};
